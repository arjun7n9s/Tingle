...
export default function ProjectPage() {
  ...
  async function refresh() {
    const res = await tingle<{ project: Project }>(`/projects/${params.id}`);
  ...
  async function send(e: FormEvent) {
    ...
    setPending(true);
    try {
      const res = await tingle<{ project: Project }>(`/projects/${params.id}/analyst`, {
        method: "POST",
  ...
  async function runPatentability() {
    ...
    try {
      const res = await tingle<{ project: Project }>(
        `/projects/${params.id}/patentability`,
  ...
  async function mute(url: string) {
    const res = await tingle<{ project: Project }>(`/projects/${params.id}/mute`, {
      method: "POST",
  ...
  async function toggleTingle(on: boolean) {
    setPending(true);
    try {
      const res = await tingle<{ project: Project }>(`/projects/${params.id}/tingle`, {
        method: "POST",
        ...
        <div className="min-w-0 space-y-8">
          <div className="tingle-mast flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-2 max-w-2xl text-[0.98rem] leading-relaxed text-[var(--muted)]">
                {project.claim}
              </p>
            </div>
          ...
          <details className="tingle-panel space-y-3">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                disabled={pending}
                onClick={() =>
                  void tingle<{ project: Project }>(`/projects/${params.id}/storage`, {
                    method: "POST",
                    body: JSON.stringify({
                      backend: "github",
                      repo,
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <button
                ...
                className="tingle-app-btn-ghost"
                onClick={() =>
                  void tingle<{ project: Project }>(`/projects/${params.id}/budget`, {
                    method: "POST",
