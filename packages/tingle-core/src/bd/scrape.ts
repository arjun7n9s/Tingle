...
import { mockBrokenChaos, mockRowsFor, mockWatchRowForUrl } from "./mock.js";
import {
  buildHealPrompt,
  isValidationSuccess,
  validateRows,
} from "./validate.js";
import { requirePinned, triggerInputs, familyOf } from "../config.js";
import { normalizeExtractorRows } from "../normalize.js";
...
export async function scrapeAndValidate(
  ...
  const normalized = await normalizeExtractorRows(family, raw, {
    llm: config.llm,
    collector: source,
  });
  raw = normalized.rows;

  let result = validateRows(family, raw);
  ...
  const prompt = (opts.healPrompt ?? buildHealPrompt(family, result.issues)).slice(
    0,
    1000,
  );
  push({
  ...
  try {
    ...
    result = validateRows(family, raw);
    if (isValidationSuccess(result)) {
      push({
  ...
  if (client.mock) {
    ...
    const base =
      opts.forceChaosBreak && source === "chaos"
        ...
        : mockRowsFor(source);
    const extraUrl =
      opts.url && source === "watch" ? [mockWatchRowForUrl(opts.url)] : [];
    return [...base, ...extraUrl, ...(opts.extraRows ?? [])];
  }
