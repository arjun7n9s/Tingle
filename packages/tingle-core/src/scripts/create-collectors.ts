...
import { COLLECTOR_SPECS } from "../collectors.js";
...
const allLanes = process.argv.includes("--all-lanes");

if (allLanes) {
  const patent = COLLECTOR_SPECS.find((s) => s.key === "patent");
  if (patent) {
    TARGETS.push({
      key: patent.key,
      name: patent.name,
...
for (const target of TARGETS) {
  ...
  const after = await client.listCollectors().catch(() => []);
  const created = after.find((c) => c.name === target.name);
  if (created) pinEnv(target.key, created.id);
  else {
    console.error(
      `create returned 0 but ${target.name} is not in the collector list yet — pin TINGLE_C_${target.key.toUpperCase()} by hand.`,
    );
  }
}
...
function runCreate(url: string, description: string, name: string): Promise<number> {
  const cmd = [
    ...
    JSON.stringify(url),
    JSON.stringify(description.slice(0, 500)),
    "--name",
    JSON.stringify(name),
    "--timeout 1500 --json",
  ].join(" ");
